"""Page number normalization and the sliding window page sequence."""
...
def normalize_page(raw_page: object, total_pages: int) -> int:
    """Turn a raw ``page`` query value into a valid 1 based page number."""
    try:
...
def page_sequence(current: int, total: int) -> list[int | str]:
    ...
    inserted where a real gap remains between two shown page numbers, and
